...
for model_name in models:
    ...
    Ds = bayes.Bayes(clone(models[model_name]),t_max=0.6,user_mode=1)
    
    tlabels = []
    plabels = []
    T_time = []
    #Bayes算法中的阈值参数，决定了输出的阈值
    ...
    for i in range(0,40):
        ...
        #调用决策函数，返回bool值和label，需要包含以下参数：
        #trail:数据
        # default_duration:默认时长，根据duration选择计算的模型
        # p_threshold:阈值，
        # filename:模型名称    
        bool,label = Ds.decide(trail,default_duration,p_threshold,filename=model_name)
        while not bool:
            # print("insufficient length, add 0.1s")
            #决策函数返回False，说明时长不够，增加0.1s
            a += 0.1
            default_duration = round(a,2)
            if model_name == 'fbtdca':
                trail = bufferX[:,:,0:int(srate*default_duration)+l]
